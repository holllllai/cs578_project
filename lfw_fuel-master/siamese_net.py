...
#print(X_train.shape)
train_pairs = np.zeros((2, 2200, feature_width, feature_height, 3))
...
test_pairs = np.zeros((2, 1000, feature_width, feature_height, 3))
...
def create_base_network(input_shape):
    net = Sequential()
    net.add(Conv2D(64, (3,3), input_shape=input_shape, strides = (2,2), padding='same', data_format='channels_last', activation='relu'))
    ...
    # net.add(Dropout(0.3))
    net.add(Flatten())
    net.add(Dense(128, activation='relu'))# kernel_regularizer = regularizers.l2(0.01)))
    net.add(BatchNormalization())
...
model.fit([train_pairs[0], train_pairs[1]], y_train, batch_size = batch_size, epochs = nb_epoch, verbose=1, validation_data=([test_pairs[0], test_pairs[1]], y_test))
...
thresholds = [1.75, 1.875, 1.9, 2.0, 2.1, 2.125, 2.25]
